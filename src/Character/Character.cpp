...
Character::Character(std::shared_ptr<MapManager> mm, std::vector<std::string> n_list, std::vector<std::string> g_list,
                     std::shared_ptr<std::unordered_map<std::string, int>> wc, bool isPlayer)
    ...
    Lv = std::stoi(n_list[CHARACTER_INDEX::LV]);
    Ex = std::stoi(n_list[CHARACTER_INDEX::EX]);
    Hp_Limit = std::stoi(n_list[CHARACTER_INDEX::HP]);
    Hp_Current = Hp_Limit;
    Str = std::stoi(n_list[CHARACTER_INDEX::STR]);
    Skl = std::stoi(n_list[CHARACTER_INDEX::SKL]);
    Wlv = std::stoi(n_list[CHARACTER_INDEX::WLV]);
    Spd = std::stoi(n_list[CHARACTER_INDEX::SPD]);
    Lck = std::stoi(n_list[CHARACTER_INDEX::LCK]);
    Def = std::stoi(n_list[CHARACTER_INDEX::DEF]);
    Res = std::stoi(n_list[CHARACTER_INDEX::RES]);
    Mov = std::stoi(n_list[CHARACTER_INDEX::MOV]);

    HPGR = std::stoi(g_list[CHARACTER_GROWTH_INDEX::HP]);
    StrGR = std::stoi(g_list[CHARACTER_GROWTH_INDEX::STR]);
    SklGR = std::stoi(g_list[CHARACTER_GROWTH_INDEX::SKL]);
    WlvGR = std::stoi(g_list[CHARACTER_GROWTH_INDEX::WLV]);
    SpdGR = std::stoi(g_list[CHARACTER_GROWTH_INDEX::SPD]);
    LckGR = std::stoi(g_list[CHARACTER_GROWTH_INDEX::LCK]);
    DefGR = std::stoi(g_list[CHARACTER_GROWTH_INDEX::DEF]);
    ResGR = std::stoi(g_list[CHARACTER_GROWTH_INDEX::RES]);
...
bool Character::walkDirectly() {
    ...
    if (p.x > absolutePos.x)
        forword = Forword::Right;
    else if (p.x < absolutePos.x)
        forword = Forword::Left;
    else if (p.y < absolutePos.y)
    ...
    glm::ivec2 step{0, 0};
    if (delta.x != 0)
        step.x = (delta.x > 0 ? 8 : -8); // 64 must be divisible by 8
    if (delta.y != 0)
        step.y = (delta.y > 0 ? 8 : -8); // 64 must be divisible by 8
    absolutePos += step;
...
void Character::buildWalkPath(glm::ivec2 a_pos) {
    ...
    std::vector<glm::ivec2> directions = {{TILE_SIZE, 0}, {0, TILE_SIZE}, {-TILE_SIZE, 0}, {0, -TILE_SIZE}};
    ...
    while (!pq.empty()) {
        ...
        for (auto &d : directions) {
            ...
            if (moveRange.find(next) == moveRange.end())
                continue;
...
void Character::findMoveRange(int mov, glm::ivec2 a_pos, std::unordered_set<glm::ivec2> mask) {
    ...
    if (p != moveRange.end() && p->second >= mov)
        return;
