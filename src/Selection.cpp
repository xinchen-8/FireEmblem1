...
bool Selection::moveJudge(Forword forword, glm::ivec2 map_size) {
    if (status != SelectionStatus::Normal && status != SelectionStatus::Moving &&
        status != SelectionStatus::AttackTargeting && status != SelectionStatus::Targeting)
        return false;

    switch (forword) {
    case Forword::Up:
        if (absolutePos.y < map_size.y - TILE_SIZE)
            return true;
        break;
    case Forword::Down:
        if (absolutePos.y > 0)
    ...
    case Forword::Left:
        ...
        break;
    case Forword::Right:
        if (absolutePos.x < map_size.x - TILE_SIZE)
            return true;
        break;
    }
    return false;
...
void Selection::moveDirectly(glm::ivec2 move) {
    if (status == SelectionStatus::Moving) {
        ...
        if (limitRange.find(newPos) != limitRange.end())
            absolutePos = newPos;
    } else if (status == SelectionStatus::AttackTargeting || status == SelectionStatus::Targeting) {
