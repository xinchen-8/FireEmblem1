...
LevelUpUI::LevelUpUI(std::vector<std::shared_ptr<Tile>>& tiles) : UserInterface(tiles) {
    setUISize({5, 7.5});
    setRelativePos({
        -floor(PTSD_Config::WINDOW_WIDTH / 2) + 8.5 * TILE_SIZE,
...
void LevelUpUI::update() {
    ...
    std::string content = 
        ...
        "HP " + std::to_string(character->getOldHpLimit()) + " -> " + std::to_string(character->getHpLimit()) + "\n" +
        "Str " + std::to_string(character->getOldStr()) + " -> " + std::to_string(character->getStr()) + "\n" +
        "Lck " + std::to_string(character->getOldLck()) + " -> " + std::to_string(character->getLck()) + "\n" +
        "Skl " + std::to_string(character->getOldSkl()) + " -> " + std::to_string(character->getSkl()) + "\n" +
        "Def " + std::to_string(character->getOldDef()) + " -> " + std::to_string(character->getDef()) + "\n" +
        "Wlv " + std::to_string(character->getOldWlv()) + " -> " + std::to_string(character->getWlv()) + "\n" +
        "Res " + std::to_string(character->getOldRes()) + " -> " + std::to_string(character->getRes()) + "\n" +
        "Spd " + std::to_string(character->getOldSpd()) + " -> " + std::to_string(character->getSpd());
    setString(content);
}
